function doGet(e) {
  try {
    ...
    if (loadType === "login") {
      ...
      const allUsers = fetchSheetData("POLICE_USER");
      const match = allUsers.find(u => (u.POLICE_EMAIL || u['E-MAIL'] || u.EMAIL || "").toLowerCase() === email);

      if (match) {
        result.user = match;
      } else {
        return json({ status: "error", message: "ไม่พบอีเมลนี้ในระบบ" });
      }
    }
    else if (loadType === "profile") {
      const code = e.parameter.policecode;
      if (code === "all") {
        result.police_users = fetchSheetData("POLICE_USER");
      } else {
        const allUsers = fetchSheetData("POLICE_USER");
        const match = allUsers.find(u => u.POLICE_CODE == code);
        if (match) {
          result.user = match;
        } else {
          return json({ status: "error", message: "ไม่พบข้อมูลผู้ใช้งาน" });
        }
      }
    }
...
function checkInOut(e) {
  try {
    ...
    const ON_DULTY_COL = 4; // D
    const currentOnDuty = rows[foundRow - 1][ON_DULTY_COL - 1]; // Array index is 0-based

    if (type === "IN") {
      if (currentOnDuty === "Y") {
        return json({ status: "error", message: "เจ้าหน้าที่อยู่ในสถานะ 'เข้าเวร' อยู่แล้ว ไม่สามารถเข้าเวรซ้ำได้", recentData: { ON_DULTY: currentOnDuty } });
      }
      ...
      if (currentOnDuty === "N") {
        return json({ status: "error", message: "เจ้าหน้าที่อยู่ในสถานะ 'ออกเวร' อยู่แล้ว ไม่สามารถออกเวรซ้ำได้", recentData: { ON_DULTY: currentOnDuty } });
      }
...
function manageEvent(data) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let sheet = ss.getSheetByName("POLICE_EVENTS");

    if (!sheet) {
      sheet = ss.insertSheet("POLICE_EVENTS", ss.getNumSheets());
      sheet.appendRow([
        "EVENT_ID", "TYPE", "TITLE", "DETAILS", "DATE",
        "START_TIME", "END_TIME", "LOCATION", "REPORTER", "CREATED_AT"
      ]);
      sheet.setFrozenRows(1);
    }
    ...
    if (data.subAction === "ADD") {
      ...
      let newId = 1001;
      if (rows.length > 1) {
        newId = Math.max(...rows.slice(1).map(r => parseInt(r[0]) || 1000)) + 1;
      }
      ...
      return json({ status: "success", message: "บันทึกภารกิจเรียบร้อยแล้ว" });
    }

    return json({ status: "error", message: "Invalid subAction" });
